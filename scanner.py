...
# POPRAWNE, BEZPIECZNE LINKI DO GIEŁD (100% działają)
EXCHANGE_LINKS = {
...
send("CEX Scanner ULTIMATE 2025 uruchomiony – wpisz /help")
    ...
    while True:
        try:
            ...
            for u in r.get("result", []):
                ...
                # tu masz wszystkie komendy – nie ruszamy
                if cmd == "/help":
...
/newlistings – nowe listingi""")
                ...
                # ... reszta komend bez zmian (skrócone dla czytelności)
...
print("CEX Scanner ULTIMATE 2025 – linki naprawione!")
                ...
                for s in pairs:
                    try:
                        ...
                        if base in watchlist or (ratio > 9 and abs(price_ch) > 5 and vol24 > MIN_VOLUME_24H):
                            ...
                            # KLUCZOWA POPRAWKA – zawsze link do właściwej giełdy
                            link_template = EXCHANGE_LINKS.get(ex.name)
                            if not link_template:
                                continue  # nie ma linku = nie wysyłamy (bezpiecznik)
                            link = link_template.replace("{base}", base)
                            ...
                            msg = f"{base}/USDT na {ex.name}\n" \
                                  ...
                                  f"Vol ×{ratio:.1f}\n" \
                                  f"{direction}\n" \
                                  f"<a href='{link}'>OTWÓRZ NA {ex.name.upper()}</a>"
